...
class Arista:
    ...
    def __init__(self, source, target, id):
        ...
        self.atrib = {
            ATTR_ESTILO: {
                ESTILO_COLOR: (200, 200, 200),
                ESTILO_GROSOR: 1,
                ESTILO_DISCONTINUO: False,
                ESTILO_TAMANO: 10,
                ESTILO_MOSTRAR_ID: False,
                ESTILO_ANTIALIAS: False,
            },
        }
    ...
    def dibujar(self, viewport, transform=None):
        ...
        if self.atrib[ATTR_ESTILO][ESTILO_DISCONTINUO]:
            dibujar_linea_punteada(surf, self.atrib[ATTR_ESTILO][ESTILO_COLOR],
                                   n0,
                                   n1,
                                   self.atrib[ATTR_ESTILO][ESTILO_GROSOR],
                                   self.atrib[ATTR_ESTILO][ESTILO_TAMANO])
        elif self.atrib[ATTR_ESTILO][ESTILO_ANTIALIAS]:
            pygame.draw.aaline(surf, self.atrib[ATTR_ESTILO][ESTILO_COLOR],
                               n0,
                               n1,
                               self.atrib[ATTR_ESTILO][ESTILO_GROSOR])
        else:
            pygame.draw.line(surf, self.atrib[ATTR_ESTILO][ESTILO_COLOR],
                             n0,
                             n1,
                             self.atrib[ATTR_ESTILO][ESTILO_GROSOR])
        ...
        try:
            if self.atrib[ATTR_ESTILO][ESTILO_MOSTRAR_ID]:
                ...
                viewport.frame.text((pos[0] - lon, pos[1] - viewport.tam_fuente / 3),
                                    self.atrib[ATTR_ESTILO][ESTILO_COLOR],
                                    str(self.id))
        except:
